...
ever17::bitblt_hook::fake_call(HDC hdc, int x, int y, int cx, int cy,
  ...
  auto res = g_ever17->m_bitblt_hook.m_old_ptr(hdc, x, y, cx, cy, hdcSrc, x1, y1, rop);
  auto *shm_ptr = g_ever17->get_shm_ptr();
  ...
  DWORD lines = g_ever17->m_bitblt_hook.m_old_ptr(hBmpDC, 0, 0, width, height, hdc, 0, 0, SRCCOPY);

  CLSID clsid;
  GetEncoderClsid(L"image/bmp", &clsid);
  
  std::wostringstream woss;
  woss << "image" << counter << ".bmp";
  std::wstring ws = woss.str();
  
  bmp->Save(ws.c_str(), &clsid, nullptr);
  counter++;
